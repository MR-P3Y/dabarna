...
def _invoice_text(item: dict, *, title: str = "فاکتور واریز رمزارز") -> str:
    ...
    if str(item.get("status") or "").upper() == "WAITING_PAYMENT":
        lines.append("\nمبلغ، شبکه و آدرس را دقیق ارسال کن. پرداخت با مبلغ متفاوت خودکار تایید نمی‌شود.")
    return panel(title, "\n".join(lines))
...
async def crypto_create_invoice(
    ...
    await message.answer(
        ...
        reply_markup=crypto_invoice_kb(int(invoice.get("id") or 0), str(invoice.get("status") or "")),
        parse_mode="HTML",
    )
...
async def crypto_refresh_status(
    ...
    await safe_edit_or_send(
        ...
        reply_markup=crypto_invoice_kb(invoice_id, str(invoice.get("status") or "")),
        parse_mode="HTML",
    )
